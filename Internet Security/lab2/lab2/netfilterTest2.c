...
unsigned int telnetFilter(void *priv, struct sk_buff *skb, const struct nf_hook_state *state) {
	...
	bool rule1 = iph->protocol == IPPROTO_TCP && tcph->dest == htons(23);
	bool rule2 = iph->protocol == IPPROTO_TCP && tcph->source == htons(23);
	bool rule3 = iph->daddr == inet_addr("128.230.18.198");
...
unsigned char getNet(void *priv, struct sk_buff *skb, const struct nf_hook_state *state) {
	return (unsigned char *)&state->net;
}
